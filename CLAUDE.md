...
- List view: `components/List/ListView.jsx` + `ListView.css` (Figma 39523:13757). Same rows as the timeline plus editable Assignee (members = me + invited emails + backend `space.members`), Due Date (native date input), Tags (`TAGS` in `constants/index.js`, colours `lv-tag-*`). Header unfold icons sort parents (children stay under their parent); the All/Done/Overdue tabs keep a parent visible when any sub-tix matches. Task fields: `assignee: {name,email}|null`, `dueDate: 'YYYY-MM-DD'|null`, `tags: string[]`.
